...
export default class MenuBuilder {
  ...
  buildMenu(config = {}) {
    ...
    this.setupContextMenu(config)

    let template

    if (process.platform === 'darwin') {
      template = this.buildDarwinTemplate(config)
    } else {
      template = this.buildDefaultTemplate(config)
    }
  ...
  buildDarwinTemplate(config = {}) {

    const { productName = 'App', isDev } = config
    ...
    const subMenuViewDev = {
      label: 'View',
      submenu: [
        { label: 'Reload', accelerator: 'Command+R', click: () => { this.mainWindow.webContents.reloadIgnoringCache()
        } },
